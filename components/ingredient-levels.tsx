...
export default function IngredientLevels({ pumpConfig, onLevelsUpdated }: IngredientLevelsProps) {
  ...
  // Überwache Änderungen in der Pumpenkonfiguration
  useEffect(() => {
    initializePumpIngredients()
  }, [pumpConfig])
  ...
  const loadLevels = async () => {
    ...
    try {
      ...
      setLevels(data)
      // Nach dem Laden der Füllstände, initialisiere auch die Pumpen-Zutaten
      await initializePumpIngredients()
    } catch (error) {
  ...
  // Hole alle Zutaten-IDs aus der Pumpenkonfiguration
  const connectedIngredientIds = pumpConfig.map((pump) => pump.ingredient)

  // Filtere Füllstände basierend auf Pumpen-Zutaten UND stelle sicher, dass alle Pumpen-Zutaten angezeigt werden
  const filteredLevels = levels.filter((level) => {
    // Zeige nur Zutaten, die an Pumpen angeschlossen sind
  ...
  // Erstelle Platzhalter für Pumpen-Zutaten, die noch keinen Füllstand haben
  const missingIngredients = connectedIngredientIds.filter(
    (ingredientId) => !levels.find((level) => level.ingredientId === ingredientId),
  )

  // Füge fehlende Zutaten als temporäre Einträge hinzu
  const allDisplayedLevels = [
    ...filteredLevels,
  ...
  const lowLevelsCount = allDisplayedLevels.filter((level) => level.currentAmount < 100).length
        ...
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="bg-[#00ff00]/10 text-[#00ff00] border-[#00ff00]/30">
              {allDisplayedLevels.length} Zutaten
            </Badge>
          </div>
        </CardHeader>
              ...
              <div className="space-y-4">
                {allDisplayedLevels.length === 0 ? (
                  ...
                  allDisplayedLevels.map((level) => {
                    ...
                    const isNew = level.currentAmount === 0 && missingIngredients.includes(level.ingredientId)

                    // Bestimme die Farbe basierend auf dem Zustand
                    const ingredient = ingredients.find((i) => i.id === level.ingredientId)
                    const isAlcoholic = ingredient?.alcoholic

                    const cardBorderColor = isNew
                      ? "border-blue-500"
                      : isCritical
                        ? "border-[#ff3b30]"
                        : isLow
                    ...
                    return (
                      ...
                      >
                        <CardContent className="p-6">
                          ...
                          {/* Warnung bei niedrigem Füllstand oder neuer Zutat */}
                          {(isLow || isNew) && (
                            <Alert
                              className={`mb-4 ${
                                isNew
                            ...
                            >
                              <AlertTriangle
                                className={`h-4 w-4 ${
                                  isNew ? "text-blue-400" : isCritical ? "text-[#ff3b30]" : "text-[#ff9500]"
                                }`}
                              />
                              <AlertDescription
                                className={`${
                              ...
                              >
                                {isNew
                                  ? "Neue Zutat! Bitte Füllstand setzen."
                                  : isCritical
                                    ? "Kritisch niedrig! Bitte nachfüllen."
